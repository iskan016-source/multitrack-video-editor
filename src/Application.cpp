...
void Application::addFilters() {
  ...
  // 
  // * time - a floating point value between 0 and 1 that represents the current time on a track. (See gradient filter)
  // * pos - the x,y position on the screen (See gradient filter)
  // * aggregateColor - the color (RGBA) that has been aggregated from all previous tracks (See Bright filter)
  // * trackColor -  the color (RGBA) of the current track texture (See Low threshold filter)
  ...
  filterPanel->addTextButton("Bright", [this]() {
    ...
    std::string code =
    "aggregateColor *= vec4(2.0, 2.0, 2.0, 1);\n"
  ...
  filterPanel->addTextButton("Gradient", [this]() {
    ...
    std::string code =
    "trackColor *= vec4(pos.x , pos.y, time, 1.0);\n";
    this->trackFilters[this->trackSelected] = code;
    this->trackShader->update(this->trackFilters);
  });
  ...
  filterPanel->addTextButton("Circle", [this]() {
    // Set the transparency the the trackColor to 0 if it is outside of the circle
  });
...
int Application::run(int argc, char *argv[]) {
  ...
  std::cout << "Timeline created with " << timeline->getTrackCount() << " track(s)" << std::endl;
  ...
  labels.push_back(new Glyph(VIEWPORT_X + 0.02, 0.0, 0.1, TITLE_HEIGHT, *viewportImage));
  delete viewportImage;

  Text exportLabel("Export", Color(255, 255, 255, 255), 40);
  Image* exportImage = exportLabel.renderToImage();
  labels.push_back(new Glyph(EXPORT_MENU_X - 0.01, 0.0, 0.09, TITLE_HEIGHT, *exportImage));
  delete exportImage;

  Text tracksLabel("Tracks", Color(255, 255, 255, 255), 40);
  Image* tracksImage = tracksLabel.renderToImage();
  labels.push_back(new Glyph(TRACK_ACTIONS_WIDTH + 0.02, TRACKS_Y, 0.1, 0.05, *tracksImage));
  delete tracksImage;
  ...
  for (int i = 0; i < assets.size(); i++) {
    ...
    IconButton *button = new IconButton(
        buttonX, buttonY, 0.1, 0.1,
        assets[i]->getThumbnail(),
        [this, i]() {
  ...
  // Create ASSET ACTIONS panel (yellow section) - now HORIZONTAL for side-by-side buttons
  assetActionsPanel = new ButtonPanel(
    0.0f,
    TITLE_HEIGHT + ASSET_THUMBNAIL_HEIGHT,
    ASSET_AREA_WIDTH,
    ASSET_ACTIONS_HEIGHT,
  ...
  assetActionsPanel->addTextButton("+ Add", [this]() {
    ...
    if (success) {
      std::cout << "Added asset " << assetSelected << " to track " << trackSelected
                << " at time " << startTime << "s (duration: " << duration << "s)" << std::endl;
    } else {
  ...
  assetActionsPanel->addTextButton("- Remove", [this]() {
    ...
    if (success) {
      std::cout << "Removed entry " << entrySelected << " from track " << trackSelected << std::endl;
  ...
  filterPanel = new ButtonPanel(
    VIEWPORT_X,
    TITLE_HEIGHT + VIEWPORT_HEIGHT,
    VIEWPORT_WIDTH,
    ASSET_ACTIONS_HEIGHT,
    Color(80, 80, 80, 255),  // Dark gray background
    ButtonPanel::LayoutDirection::HORIZONTAL
  ...
  Button *play = new Button(
      VIEWPORT_X + VIEWPORT_WIDTH/2 - 0.025f,  // Centered
      TITLE_HEIGHT + VIEWPORT_HEIGHT - 0.12f,   // Bottom of viewport
      0.05, 0.1,
      Image("assets/images/play.png"), [this]() {
  ...
  trackActionsPanel->addTextButton("+ New Track", [this]() {
    ...
    std::cout << "Created track " << trackIndex
              << ", now using track " << trackSelected << std::endl;
  ...
  exportMenuView = new ExportMenuView(
    EXPORT_MENU_X,
    TITLE_HEIGHT,
    EXPORT_MENU_WIDTH,
    VIEWPORT_HEIGHT,
  ...
  exportMenuController = new ExportMenuController(
    exportMenuModel,
    exportMenuView,
    exportFacade
  );
  ...
  video = new Glyph(VIEWPORT_X, TITLE_HEIGHT, VIEWPORT_WIDTH, VIEWPORT_HEIGHT, trackShader);
  ...
  while (!glfwWindowShouldClose(window->getWindow())) {
    ...
    // -------------------------------------
    {
      ...
      if (trackVisualization) {
        trackVisualization->draw(trackSelected, entrySelected, timeline->getCurrentTime());
      }
...
void Application::onMouseMove(float x, float y) {
  ...
  if (isDraggingEntry && trackVisualization) {
    ...
    double timeOffset = trackVisualization->getTimeAtPosition(x + 0.001f) -
...
void Application::onMouseClick(int button, int action, int mods) {
  ...
  // Handle track visualization clicks for track/entry selection, dragging, and resizing
  if (action == GLFW_PRESS && trackVisualization) {
    ...
    if (trackVisualization->isNearEntryRightEdge(x, y, clickedTrack, clickedEntry)) {
      ...
      if (track && entrySelected >= 0 && entrySelected < track->getEntryCount()) {
        ...
        std::cout << "Started resizing entry " << clickedEntry << " on track " << clickedTrack << std::endl;
      }
      return;  // Don't process other clicks
    }

    // Otherwise check if clicking on entry body (drag handle)
    if (trackVisualization->getEntryAtPosition(x, y, clickedTrack, clickedEntry)) {
      // Clicked on an entry
      trackSelected = clickedTrack;
      entrySelected = clickedEntry;
      ...
      if (track && entrySelected >= 0 && entrySelected < track->getEntryCount()) {
        ...
        std::cout << "Started dragging entry " << clickedEntry << " on track " << clickedTrack << std::endl;
      }
      return;  // Don't process other clicks
    }
